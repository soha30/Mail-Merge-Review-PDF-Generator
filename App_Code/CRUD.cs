   ...
   public  class CRUD
    {
        ...
        public static string conStr = WebConfigurationManager.ConnectionStrings["student2ConStr"].ConnectionString;
        // I create a con object once and then use it in methods
        public SqlConnection con = new SqlConnection(WebConfigurationManager.ConnectionStrings["student2ConStr"].ConnectionString);

        // working with excel datasource new Sep 2023
        public static string pathExcel = @"C:\ds\ds_excelStudent";
        public static string conStrExcel = "Provider=Microsoft.ACE.OLEDB.12.0;Data Source;=" + pathExcel + ";Extended Properties=Excel 12.0;";
        ...
        public DataSet getDS(string mySql) // rtn to work on it for optimization   //11
        {
            try
            {
                cmd = new SqlCommand();
                ds = new DataSet();
                adp = new SqlDataAdapter();
                con = new SqlConnection(conStrExcel);  // ConnectionString changed to Global.DB_CONN_STR
                cmd.Connection = con;
                if (cmd.Connection.State == ConnectionState.Closed)
                    using (con)
                    {
